...
- CORS landmine: `/api/*` reflects `Access-Control-Allow-Origin` only for origins in `CLERK_AUTHORIZED_PARTIES` (env var overrides the `ws-bridge.js` default; same var drives Clerk `azp`). A frontend-domain rename must add the new origin to *both*, or every API call is CORS-blocked. See root `CLAUDE.md` Deployment for the full note.
