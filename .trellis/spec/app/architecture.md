...
- Frontend → Vercel serves `app/` statically (`fourier-tutor.vercel.app`, renamed 2026-07-06 from `aquarius-seven.vercel.app`), talks to Render via `app/config.js`. **Pushing origin/main auto-deploys Vercel (~1 min).**
...
- CORS landmine: `/api/*` reflects `Access-Control-Allow-Origin` only for origins accepted by `isAllowedOrigin()` in `ws-bridge.js` — the exact `CLERK_AUTHORIZED_PARTIES` list (env var overrides the default; same list drives Clerk `azp`) **or** `VERCEL_PREVIEW_ORIGIN_RE`, a team-scoped regex covering `fourier-<hash>-mrparameciums-projects.vercel.app` preview deploys. The one matcher feeds both CORS and the `azp` check (`clerk-verify.js` takes an injected `isAuthorizedParty` predicate). A frontend-domain rename must add the new origin to *both* the default and the Render env, or every API call is CORS-blocked. See root `CLAUDE.md` Deployment for the full note.
